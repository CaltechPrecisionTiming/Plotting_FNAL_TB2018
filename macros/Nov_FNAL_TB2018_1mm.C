...
void Nov_FNAL_TB2018_1mm(const std::string& inFileName, const std::string& ch2  = "0", const std::string& ch1  = "16", const std::string& ch1_tl  = "16", const std::string& time_ch1  = "LP2_10", float amp_low_ch2 = 20.0, float amp_high_ch2 = 300.0, float amp_low = 200.0, float amp_high = 850.0, float x_tile_low = 20.0, float x_tile_high = 30.0, float y_tile_low = 5.0, float y_tile_high = 12.0, float x_sipm_low = 23.0, float x_sipm_high = 27.0, float y_sipm_low = 7.0, float y_sipm_high = 10.0, const std::string& analysis_tag="")
{
	...
	string cut_time = "&& abs("+time_ch1+"["+ch1_tl+"])>0.1 && abs("+time_ch2+"["+ch2+"])>0.1";	

	string cut_noPos = "amp["+ch1+"]>"+std::to_string(amp_low)+" && amp["+ch1+"] < "+std::to_string(amp_high)+" && amp["+ch2+"]>"+std::to_string(amp_low_ch2)+" && amp["+ch2+"]<"+std::to_string(amp_high_ch2)+"&& ntracks == 1 && chi2 > -0.1 && chi2 < 50.0" + cut_time;
        ...
        string cut_sample = "amp["+ch1+"]>"+std::to_string(amp_low)+" && amp["+ch1+"] < "+std::to_string(amp_high)+" && amp["+ch2+"]>"+std::to_string(amp_low_ch2)+" && amp["+ch2+"]<"+std::to_string(amp_high_ch2)+" && x_dut[0] > "+std::to_string(x_sample_low)+" && x_dut[0]<"+std::to_string(x_sample_high)+" && y_dut[0]>"+std::to_string(y_sample_low)+" && y_dut[0]<"+std::to_string(y_sample_high)+"&& ntracks == 1 && chi2 > -0.1 && chi2 < 50.0" + cut_time;
